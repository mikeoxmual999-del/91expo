...
function SuccessContent() {
  ...
  useEffect(() => {
    if (!caseId || !plan || !duration || !paymentIntentId) {
      setStatus("failed");
      return;
    }
      ...
      try {
        ...
        if (stored) {
          ...
          if (cases[caseId]) {
            ...
            cases[caseId].expiresAt = expiresAt;
        ...
        await fetch("/api/cases", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id: caseId, status: "未回应", paid: true, plan, duration, expires_at: expiresAt }),
        }).catch(console.error);
    ...
    verify();
  }, [caseId, plan, duration, paymentIntentId]);

  if (status === "loading") return (
    <main className="min-h-screen bg-[#F5F7FA] flex items-center justify-center">
      <div className="text-[#6B7280] text-sm">验证付款中...</div>
    </main>
  );
  ...
  if (status === "failed") return (
    <main className="min-h-screen bg-[#F5F7FA] flex items-center justify-center">
      <div className="max-w-[500px] mx-auto px-8 text-center">
        <div className="w-20 h-20 rounded-full bg-red-100 border border-red-200 flex items-center justify-center mx-auto mb-8">
        ...
        <h1 className="text-3xl font-bold text-[#0F2A44] mb-3">付款未完成</h1>
        <p className="text-[#6B7280] mb-6">您的付款尚未成功，记录未发布。</p>
