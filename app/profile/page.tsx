      ...
      <div className="max-w-[1000px] mx-auto px-8 py-16">
        ...
        <div className="bg-[#111827] border border-white/10 rounded-xl p-6 mb-10 flex items-center justify-between">
          ...
          <button
            ...
            className="bg-red-500/20 hover:bg-red-500/40 border border-red-500/30 text-red-400 px-5 py-2 rounded-lg text-sm transition"
          >
            退出登录
          </button>
        </div>
              ...
              {unpaidCases.map((c) => (
                <div key={c.id} className="bg-[#111827] border border-yellow-500/30 rounded-xl p-6">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="text-xs px-3 py-1 rounded-full text-yellow-400 bg-yellow-500/10 border border-yellow-500/20">
                      待付款
                    </div>
                  </div>
                  ...
                  <Link
                    href={`/pricing?caseId=${c.id}`}
                    className="inline-block bg-yellow-500 hover:bg-yellow-400 text-black px-5 py-2 rounded-lg text-sm font-medium transition"
                  >
                    去付款 →
                  </Link>
                </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            ...
            <Link
              href="/create"
              className="text-sm bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded-lg transition"
            >
              + 发布新记录
            </Link>
          </div>
          ...
          {paidCases.length === 0 && unpaidCases.length === 0 && (
            <div className="text-center py-20 bg-[#111827] border border-white/10 rounded-xl">
              ...
              <Link
                href="/create"
                className="inline-block text-sm bg-blue-600 hover:bg-blue-500 px-6 py-2.5 rounded-xl transition"
              >
                发布第一条记录
              </Link>
            </div>
            ...
            {paidCases.map((c) => (
              <div key={c.id} className="bg-[#111827] border border-white/10 rounded-xl p-6">
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className={`text-xs px-3 py-1 rounded-full ${statusColor(c.status)}`}>
                    {c.status}
                  </div>
                </div>
                ...
                <button
                  ...
                  className="text-sm px-4 py-2 border border-white/20 rounded-lg hover:border-white/40 transition"
                >
                  查看详情
                </button>
