  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (caseRes.ok) {
        ...
        const timeline = [...timelineArr, `🤝 ${maskedUser} 提交协调请求 · 期望金额：${amount || "未填写"} · ${now}`];
        ...
        if (caseData.creator && caseData.creator !== user) {
          const dmText = `您好，有用户对案件「${caseData.company}」提交了协调请求。\n\n期望解决金额：${amount || "未填写"}\n补充说明：${desc || "无"}\n\n请登录平台查看详情并回应。`;
          await fetch("/api/messages", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              caseId,
              posterId: caseData.creator,
              responderId: user,
              sender: user,
              text: dmText,
            }),
    ...
    if (caseId && cases[caseId]) {
      ...
      cases[caseId].timeline.push(`🤝 ${maskedUser} 提交协调请求 · 期望金额：${amount || "未填写"} · ${now}`);
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-8 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-xs text-blue-600 leading-relaxed">
              提交后系统将自动通知案件发布方，并由平台在 1-3 个工作日内协助推进协调流程。案件状态将更新为「协商中」。
            </div>
